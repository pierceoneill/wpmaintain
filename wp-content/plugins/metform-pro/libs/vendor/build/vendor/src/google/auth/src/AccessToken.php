...
use DateTime;
use Exception;
use MetFormProVendor\Firebase\JWT\ExpiredException;
use MetFormProVendor\Firebase\JWT\JWT;
use MetFormProVendor\Firebase\JWT\SignatureInvalidException;
use MetFormProVendor\Google\Auth\Cache\MemoryCacheItemPool;
use MetFormProVendor\Google\Auth\HttpHandler\HttpClientCache;
use MetFormProVendor\Google\Auth\HttpHandler\HttpHandlerFactory;
use MetFormProVendor\GuzzleHttp\Psr7;
use MetFormProVendor\GuzzleHttp\Psr7\Request;
use InvalidArgumentException;
use MetFormProVendor\phpseclib\Crypt\RSA;
use MetFormProVendor\phpseclib\Math\BigInteger;
use MetFormProVendor\Psr\Cache\CacheItemPoolInterface;
use RuntimeException;
use MetFormProVendor\SimpleJWT\InvalidTokenException;
use MetFormProVendor\SimpleJWT\JWT as SimpleJWT;
use MetFormProVendor\SimpleJWT\Keys\KeyFactory;
use MetFormProVendor\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
    ...
    public function verify($token, array $options = [])
    {
        ...
        try {
            ...
            // firebase/php-jwt 3+
        } catch (\MetFormProVendor\ExpiredException $e) {
            // firebase/php-jwt 2
        } catch (SignatureInvalidException $e) {
            // firebase/php-jwt 3+
        } catch (\MetFormProVendor\SignatureInvalidException $e) {
            // firebase/php-jwt 2
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists('MetFormProVendor\\SimpleJWT\\JWT')) {
    ...
    {
        $class = \class_exists('MetFormProVendor\\Firebase\\JWT\\JWT') ? 'Firebase\\JWT\\JWT' : 'JWT';
