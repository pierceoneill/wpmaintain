...
use MetFormProVendor\Psr\Http\Message\ResponseInterface;
use MetFormProVendor\Psr\Http\Message\UriInterface;
/**
...
class OAuth2 implements FetchAuthTokenInterface
{
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        if (\class_exists('MetFormProVendor\\Firebase\\JWT\\JWT')) {
            return \MetFormProVendor\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
        }
        return \MetFormProVendor\JWT::decode($idToken, $publicKey, $allowedAlgs);
    }
    private function jwtEncode($assertion, $signingKey, $signingAlgorithm, $signingKeyId = null)
    {
        if (\class_exists('MetFormProVendor\\Firebase\\JWT\\JWT')) {
            return \MetFormProVendor\Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
        }
        return \MetFormProVendor\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
    }
    /**
