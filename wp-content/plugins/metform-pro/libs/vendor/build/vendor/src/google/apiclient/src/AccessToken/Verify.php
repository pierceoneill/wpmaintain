 ...
 */
namespace MetFormProVendor\Google\AccessToken;

use MetFormProVendor\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use MetFormProVendor\Firebase\JWT\SignatureInvalidException;
use MetFormProVendor\GuzzleHttp\Client;
use MetFormProVendor\GuzzleHttp\ClientInterface;
use MetFormProVendor\phpseclib3\Crypt\PublicKeyLoader;
use MetFormProVendor\phpseclib3\Crypt\RSA\PublicKey;
use MetFormProVendor\Psr\Cache\CacheItemPoolInterface;
use MetFormProVendor\Google\Auth\Cache\MemoryCacheItemPool;
use MetFormProVendor\Google\Exception as GoogleException;
use MetFormProVendor\Stash\Driver\FileSystem;
use MetFormProVendor\Stash\Pool;
use DateTime;
use DomainException;
use Exception;
use MetFormProVendor\ExpiredException;
// Firebase v2
use LogicException;
/**
...
class Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('MetFormProVendor\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'MetFormProVendor\\Firebase\\JWT\\JWT';
        }
    ...
    private function getPublicKey($cert)
    {
        ...
        if (\class_exists('MetFormProVendor\\phpseclib3\\Crypt\\RSA\\PublicKey')) {
    ...
    {
        if (\class_exists('MetFormProVendor\\phpseclib3\\Crypt\\RSA')) {
            return 'MetFormProVendor\\phpseclib3\\Crypt\\RSA';
        }
        if (\class_exists('MetFormProVendor\\phpseclib\\Crypt\\RSA')) {
            return 'MetFormProVendor\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('MetFormProVendor\\phpseclib3\\Math\\BigInteger')) {
            return 'MetFormProVendor\\phpseclib3\\Math\\BigInteger';
        }
        if (\class_exists('MetFormProVendor\\phpseclib\\Math\\BigInteger')) {
            return 'MetFormProVendor\\phpseclib\\Math\\BigInteger';
        }
        return 'Math_BigInteger';
    }
    private function getOpenSslConstant()
    {
        if (\class_exists('MetFormProVendor\\phpseclib3\\Crypt\\AES')) {
            return 'phpseclib3\\Crypt\\AES::ENGINE_OPENSSL';
        }
        if (\class_exists('MetFormProVendor\\phpseclib\\Crypt\\RSA')) {
            return 'phpseclib\\Crypt\\RSA::MODE_OPENSSL';
        }
        if (\class_exists('MetFormProVendor\\Crypt_RSA')) {
