...
class MonsterInsights_SharedCount {
	...
	public function ajax_start_indexing() {
		...
		wp_send_json_error( array(
			'message' => esc_html__( 'The SharedCount API key is not set', 'ga-premium' ),
		) );
	...
	public function handle_api_error( $parsed_response ) {
		if ( isset( $parsed_response['Error'] ) && isset( $parsed_response['Type'] ) && 'invalid_api_key' === $parsed_response['Type'] ) {
			$error = esc_html__( 'The SharedCount API key is invalid', 'ga-premium' );
		} elseif ( ! empty( $parsed_response['quota_exceeded'] ) ) {
			$error = $parsed_response['quota_exceeded'];
		} else {
			$error = isset( $parsed_response['Error'] ) ? $parsed_response['Error'] : esc_html__( 'There was an error grabbing data from SharedCount, please check the API Key', 'ga-premium' );
		}
		$this->error = $error;
	}
