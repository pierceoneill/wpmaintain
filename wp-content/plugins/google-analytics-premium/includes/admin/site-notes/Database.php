...
class MonsterInsights_Site_Notes_DB_Base
{
	...
	public function insert_default_categories()
	{
		...
		$categories = array(
			__('Website Updates', 'ga-premium'),
			__('Blog Post', 'ga-premium'),
			__('Promotion', 'ga-premium'),
		);
	...
	{
		if (empty($data['note'])) {
			return new WP_Error(400, __('Your Site Note Cannot be Empty', 'ga-premium'));
		}
	...
	public function get($post_id)
	{
		...
		if (!$post) {
			return new WP_Error(400, __('Note not found', 'ga-premium'));
		}
