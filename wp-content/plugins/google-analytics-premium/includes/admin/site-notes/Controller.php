	...
	public function get_notes() {
		...
		if ( ! current_user_can( 'monsterinsights_view_dashboard' ) && ! current_user_can( 'monsterinsights_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to view notes.", 'ga-premium' ),
				)
			);
		}
	...
	public function get_categories() {
		...
		if ( ! current_user_can( 'monsterinsights_view_dashboard' ) && ! current_user_can( 'monsterinsights_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to view notes categories.", 'ga-premium' ),
				)
			);
		}
	...
	public function save_note() {
		...
		if ( ! current_user_can( 'monsterinsights_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to update notes.", 'ga-premium' ),
				)
			);
		}
	...
	public function save_category() {
		...
		if ( ! current_user_can( 'monsterinsights_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to update categories.", 'ga-premium' ),
				)
			);
		}
		...
		if (empty($category->name)) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __('Please add a category name', 'ga-premium'),
				)
			);
		}
		...
		if (200 < mb_strlen($category->name)) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __('You can\'t exceed the 200 characters length for each site note.', 'ga-premium'),
				)
			);
		}
	...
	public function trash_notes() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a site note to trash!', 'ga-premium'),
				)
			);
		}
	...
	public function restore_notes() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a site note(s) to restore!', 'ga-premium'),
				)
			);
		}
	...
	public function delete_notes() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a site note(s) to delete!', 'ga-premium'),
				)
			);
		}
	...
	public function delete_categories() {
		...
		if (empty($ids)) {
			wp_send_json(
				array(
					'success' => false,
					'message' => __('Please choose a category to delete!', 'ga-premium'),
				)
			);
		}
		...
		<div class="monsterinsights-metabox" id="monsterinsights-metabox-site-notes">
			<div class="monsterinsights-metabox-input monsterinsights-metabox-input-checkbox">
				<label class="">
					...
					<span class="monsterinsights-metabox-input-checkbox-label"><?php _e('Add a Site Note', 'ga-premium'); ?></span>
				</label>
			</div>
