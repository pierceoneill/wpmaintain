...
class MonsterInsights_Updater {
	...
	public function maybe_show_license_expired_message( $plugin_data, $response ) {
		...
		if ( empty( $response->package ) ) {
			...
			echo '<br />' . sprintf( __( 'In order to enable updates, you need to have a valid license key on the %1$ssettings page%2$s. If your license key is expired or you need a new key, then %3$sclick here to purchase MonsterInsights Pro%2$s.', 'ga-premium' ), '<a href="' . esc_url( $settings_url ) . '">', '</a>', '<a href="' . monsterinsights_get_url( 'Plugin update', $this->plugin_name ) . '">' ); // phpcs:ignore
		}
	}
