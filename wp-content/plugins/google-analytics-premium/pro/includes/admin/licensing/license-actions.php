...
final class MonsterInsights_License_Actions {
	...
	public function ajax_verify_key() {
		...
		if ( ! $license ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'Please enter a license key.', 'ga-premium' ),
				)
			);
		}
		...
		if ( ! current_user_can( 'monsterinsights_save_settings' ) ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'You are not allowed to verify a license key.', 'ga-premium' ),
				)
			);
		}
		...
		if ( ! $verify ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'There was an error connecting to the remote key API. Please try again later.', 'ga-premium' ),
				)
			);
		}
		...
		$message = isset( $verify->success ) ? $verify->success : esc_html__( 'Congratulations! This site is now receiving automatic updates.', 'ga-premium' );
	...
	public function verify_key( $key = '' ) {
		...
		if ( ! $verify ) {
			$this->errors[] = esc_html__( 'There was an error connecting to the remote key API. Please try again later.', 'ga-premium' );
		...
		$option['is_invalid']  = false;
		$option['is_agency']   = isset( $verify->is_agency ) ? boolval( $verify->is_agency ) : false;
		$this->success[]       = isset( $verify->success ) ? $verify->success : esc_html__( 'Congratulations! This site is now receiving automatic updates.', 'ga-premium' );
	...
	public function validate_key( $forced = false, $network = null ) {
		...
		if ( isset( $validate->key ) || isset( $validate->author ) ) {
			...
			$this->errors[] = esc_html__( 'Your license key for MonsterInsights is invalid. The key no longer exists or the user associated with the key has been deleted. Please use a different key.', 'ga-premium' );
		...
		if ( isset( $validate->expired ) ) {
			...
			$this->errors[] = sprintf( esc_html__( 'Your license key for MonsterInsights has expired. %1$sPlease click here to renew your license key.%2$s', 'ga-premium' ), '<a href="' . monsterinsights_get_url( 'admin-notices', 'expired-license', 'https://www.monsterinsights.com/login/' ) . '" target="_blank" rel="noopener noreferrer" referrer="no-referrer">', '</a>' );
		...
		if ( isset( $validate->disabled ) ) {
			...
			$network ? MonsterInsights()->license->set_network_license( $option ) : MonsterInsights()->license->set_site_license( $option );
			$this->errors[] = esc_html__( 'Your license key for MonsterInsights has been disabled. Please use a different key.', 'ga-premium' );
		...
		if ( ( ! empty( $validate->key ) || ! empty( $this->key ) ) && $forced ) {
			...
			$this->success[] = esc_html__( 'Congratulations! Your key has been refreshed successfully.', 'ga-premium' );
		}
	...
	public function ajax_deactivate_key() {
		...
		if ( ! $license ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'Please refresh the page and try again.', 'ga-premium' ),
				)
			);
		}
		...
		if ( ! current_user_can( 'monsterinsights_save_settings' ) ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'You are not allowed to deactivate the license key.', 'ga-premium' ),
				)
			);
		}
		...
		wp_send_json_success(
			array(
				'message' => isset( $deactivate->success ) ? $deactivate->success : esc_html__( 'Congratulations! You have deactivated the key from this site successfully.', 'ga-premium' ),
			)
		);
	}
	...
	public function deactivate_key() {
		...
		$this->success[] = isset( $deactivate->success ) ? $deactivate->success : esc_html__( 'Congratulations! You have deactivated the key from this site successfully.', 'ga-premium' );
