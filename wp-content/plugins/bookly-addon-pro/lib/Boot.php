...
class Boot
{
    ...
    public static $req_version = '24.6';
    ...
    public static function up()
    {
        ...
        add_filter( 'plugins_loaded', function() {
            if ( class_exists( '\Bookly\Lib\Plugin', false ) ) {
                $plugins_dir = WP_CONTENT_DIR . '/plugins/';
                foreach ( \Bookly\Lib\Config::getProductsX() as $slug ) {
                    $namespace = implode( '', array_map( 'ucfirst', explode( '-', str_replace( '-addon-', '-', $slug ) ) ) );
                    $external_addon_main_file = $plugins_dir . $slug . '/main.php';
                    $product_x_main_file = $plugins_dir . 'bookly-addon-pro/addons/' . $slug . '/main.php';
                    register_activation_hook( $product_x_main_file, function( $network_wide ) use ( $namespace ) {
                        $plugin = '\\' . $namespace . '\Lib\Plugin';
                        $boot = '\\' . $namespace . '\Lib\Boot';
                        if ( $boot::checkEnv() ) {
                            $plugin::activate( $network_wide );
                        }
                    } );
                    register_deactivation_hook( $external_addon_main_file, function( $network_wide ) use ( $namespace ) {
                        $plugin = '\\' . $namespace . '\Lib\Plugin';
                        $boot = '\\' . $namespace . '\Lib\Boot';
                        if ( $boot::checkEnv() ) {
                            $plugin::deactivate( $network_wide );
                    ...
                    register_uninstall_hook( $external_addon_main_file, '__return_null' );
                }
            }
        } );
