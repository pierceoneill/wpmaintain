...
abstract class Config extends Cache
{
        ...
        if ( self::$grace_remaining_days === null ) {
            ...
            if ( $api_error_day && $today - $api_error_day > 7 ) {
                ...
                foreach ( $addons as $plugin_class ) {
                    ...
                    if ( $plugin_class::getPurchaseCode() != '' ) {
                        continue;
                    }
                    if ( $plugin_class::embedded() ) {
                        if ( self::isActive( $plugin_class ) ) {
                            continue;
                        }
                        if ( in_array( $plugin_class::getSlug(), $licensed_products ) ) {
                            continue;
                        }
                    }
     ...
     * @return bool
     */
    public static function isActive( $plugin_class )
    {
        static $active_products;

        if ( $active_products === null ) {
            $active_products = get_option( 'bookly_cloud_account_products' ) ?: array();
        }

        if ( ! self::hasInCache( $plugin_class ) ) {
            $products = BooklyLib\Config::getProductsX();

            $found = in_array( $plugin_class::getSlug(), $products, false );
            $active = false;
            if ( $found ) {
                foreach ( $products as $product => $slug ) {
                    if ( in_array( $product, $active_products, true ) ) {
                        $active = true;
                        break;
                    }
                }
            ...
            self::putInCache( $plugin_class, $active );
        }

        return self::getFromCache( $plugin_class );
    }
