...
class Local extends Proxy\Pro
{
    ...
    {
        $min_time_prior_bc = array( array( null, __( 'Default', 'bookly' ) ), array( '0', __( 'Disabled', 'bookly' ) ) );
        foreach ( array_merge( array( 0.5 ), range( 1, 12 ), range( 24, 144, 24 ), range( 168, 672, 168 ) ) as $hour ) {
            $min_time_prior_bc[] = array( $hour * HOUR_IN_SECONDS, BooklyLib\Utils\DateTime::secondsToInterval( $hour * HOUR_IN_SECONDS ) );
        }

        return self::renderTemplate( 'advanced_settings', compact( 'service', 'min_time_prior_bc' ), false );
    }
