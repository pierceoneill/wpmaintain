...
if ( ! class_exists( 'bookingpress_pro_payment_gateways' ) ) {
	class bookingpress_pro_payment_gateways Extends BookingPress_Core {
		function __construct() {
			...
			add_filter( 'bookingpress_modify_cart_entry_data_before_insert', array( $this, 'bookingpress_modify_cart_entry_data_before_insert_func_for_overnight' ), 10, 3 );
		}
		...
		*/
		...
		function bookingpress_modify_cart_entry_data_before_insert_func_for_overnight( $bookingpress_entry_details, $posted_data,$v ){

			if( empty( $bookingpress_entry_details['bookingpress_appointment_end_date'] ) ){
				$bookingpress_entry_details['bookingpress_appointment_end_date'] = !empty( $v['bookingpress_selected_end_date'] ) ? $v['bookingpress_selected_end_date'] : $bookingpress_entry_details['bookingpress_appointment_date'];
			}

			if( empty( $bookingpress_entry_details['bookingpress_is_next_day_booking'] ) ){
				$bookingpress_entry_details['bookingpress_is_next_day_booking'] = !empty( $v['is_both_next_day'] ) ? $v['is_both_next_day'] : 00;
			}
		...
		function bookingpress_validate_submitted_booking_form_func( $payment_gateway, $posted_data ) {
			...
			if ( ! empty( $posted_data ) && ! empty( $payment_gateway ) && empty($bookingpress_appointment_data['cart_items']) && $bookingpress_add_single_entry ) {
				...
				$bookingpress_appointment_booked_date = sanitize_text_field( $bookingpress_appointment_data['store_selected_date'] );
				$bookingpress_appointment_booked_end_date = !empty( $bookingpress_appointment_data['store_selected_end_date'] ) ? $bookingpress_appointment_data['store_selected_end_date'] : $bookingpress_appointment_booked_date;
				$bookingpress_selected_start_time     = !empty( $bookingpress_appointment_data['store_start_time'] ) ? sanitize_text_field( $bookingpress_appointment_data['store_start_time'] ) : '00:00:00';
				$bookingpress_selected_end_time       = !empty( $bookingpress_appointment_data['store_end_time'] ) ? sanitize_text_field( $bookingpress_appointment_data['store_end_time'] ) : '00:00:00';
				...
				$bookingpress_is_customer_next_day_booking = ( !empty( $bookingpress_appointment_data['is_customer_both_next_day'] ) && ( true === $bookingpress_appointment_data['is_customer_both_next_day'] || 'true' == $bookingpress_appointment_data['is_customer_both_next_day'] ) ) ? 1 : 0;

				$bookingpress_selected_appointment_booked_date = !empty( $bookingpress_appointment_data['customer_selected_date'] ) ? sanitize_text_field( $bookingpress_appointment_data['customer_selected_date'] ) : $bookingpress_appointment_booked_date;				
				$bookingpress_selected_appointment_booked_end_date = !empty( $bookingpress_appointment_data['customer_selected_end_date'] ) ? sanitize_text_field( $bookingpress_appointment_data['customer_selected_end_date'] ) : $bookingpress_appointment_booked_date;				
				$bookingpress_selected_appointment_booked_time = !empty( $bookingpress_appointment_data['customer_selected_time'] ) ? sanitize_text_field( $bookingpress_appointment_data['customer_selected_time'] ) : $bookingpress_selected_start_time;				
				$bookingpress_selected_appointment_booked_end_time = !empty( $bookingpress_appointment_data['customer_selected_end_time'] ) ? sanitize_text_field( $bookingpress_appointment_data['customer_selected_end_time'] ) : $bookingpress_selected_end_time;

				$bookingpress_appointment_data['bookingpress_customer_timezone'] = ( isset( $bookingpress_appointment_data['client_offset'] ) && '' !== $bookingpress_appointment_data['client_offset'] ) ? $bookingpress_appointment_data['client_offset'] : '';
				...
				$customer_timezone = ( isset($bookingpress_appointment_data['bookingpress_customer_timezone']) && '' !== $bookingpress_appointment_data['bookingpress_customer_timezone'] ) ? $bookingpress_appointment_data['bookingpress_customer_timezone'] : wp_timezone_string();
				...
				if($payment_gateway != "on-site" && $payment_gateway != " - " && $bookingpress_deposit_payment->bookingpress_check_deposit_payment_module_activation() && !empty($bookingpress_appointment_data['bookingpress_deposit_payment_method']) && ($bookingpress_appointment_data['bookingpress_deposit_payment_method'] == "deposit_or_full_price") && $bookingpress_appointment_data['bookingpress_remove_deposit'] != 1 ){
					...
					$deposit_value = (isset($bookingpress_appointment_data['deposit_payment_amount']) && !empty($bookingpress_appointment_data['deposit_payment_amount'])) ? $bookingpress_appointment_data['deposit_payment_amount'] : '';
					$bookingpress_deposit_details = array(
				...
				$bookingpress_entry_details = array(
					...
					'bookingpress_is_next_day_booking' 			  => $bookingpress_is_next_day_booking,
					...
					'bookingpress_selected_appointment_date'	  => $bookingpress_selected_appointment_booked_date,
					'bookingpress_selected_appointment_end_date'  => $bookingpress_selected_appointment_booked_end_date,
					'bookingpress_selected_appointment_time'	  => $bookingpress_selected_appointment_booked_time,
					'bookingpress_selected_appointment_end_time'  => $bookingpress_selected_appointment_booked_end_time,
					'bookingpress_customer_is_next_day_booking'   => $bookingpress_is_customer_next_day_booking,
		...
		public function bookingpress_confirm_booking( $entry_id, $payment_gateway_data, $payment_status, $transaction_id_field = '', $payment_amount_field = '', $is_front = 2, $is_cart_order = 0, $payment_currency_field = '' ) {
			...
			if($bookingpress_booked_appointment_id > 0){
				...
				if($bookingpress_entry_id){
					...
					if ( ! empty( $entry_data ) ) { 
						...
						/** Update coupon usage if found */
						$bpa_coupon_details = json_decode( $bookingpress_coupon_details, true );
						if( !empty( $bpa_coupon_details['coupon_data'] ) && !empty( $bookingpress_coupons ) && $bookingpress_coupons->bookingpress_check_coupon_module_activation() ){
							$coupon_data = $bpa_coupon_details['coupon_data'];

							$coupon_id = $coupon_data['bookingpress_coupon_id'];
				...
				$bookingpress_is_group_order = apply_filters('bookingpress_check_is_group_order_for_complete_payment_update',false, $bookingpress_booked_appointment_id,$bookingpress_order_id);
				...
				if($bookingpress_is_cart || $bookingpress_is_group_order){		
					...
					$bookingpress_appointment_booking_update_data['bookingpress_complete_payment_token'] = '';
					$bookingpress_appointment_booking_update_data['bookingpress_appointment_status'] = $bookingpress_ap_status;	
					$wpdb->update($tbl_bookingpress_appointment_bookings, $bookingpress_appointment_booking_update_data, array('bookingpress_appointment_booking_id' => $bookingpress_booked_appointment_id) );
					$bookingpress_payment_log_update_data['bookingpress_complete_payment_token'] = '';
					$bookingpress_payment_log_update_data['bookingpress_payment_status'] = 1;
					$bookingpress_payment_log_update_data['bookingpress_transaction_id'] = $transaction_id;
					$bookingpress_payment_log_update_data['bookingpress_payment_gateway'] = $payment_gateway_name;
					...
					$wpdb->update($tbl_bookingpress_payment_logs, $bookingpress_payment_log_update_data, array('bookingpress_appointment_booking_ref' => $bookingpress_booked_appointment_id) );					
					if( empty($old_appointment_status) || ( $old_appointment_status != $bookingpress_ap_status ) ){
						do_action('bookingpress_after_change_appointment_status', $bookingpress_booked_appointment_id, $bookingpress_ap_status);
						$bookingpress_email_notifications->bookingpress_send_after_payment_log_entry_email_notification( $bookingpress_email_notification_type, $bookingpress_booked_appointment_id, 
			...
			if ( ! empty( $entry_id ) && empty($is_cart_order) ) {
				...
				if ( ! empty( $entry_data ) ) {
					...
					$bookingpress_appointment_end_date           = $entry_data['bookingpress_appointment_end_date'];
					$bookingpress_is_next_day_booking 			 = $entry_data['bookingpress_is_next_day_booking'];
					...
					$bookingpress_is_customer_next_day_booking   = $entry_data['bookingpress_customer_is_next_day_booking'];
					...
					$bpa_appointemnt_data_form_fields = !empty( $_REQUEST['appointment_data']['form_fields'] ) ? $_REQUEST['appointment_data']['form_fields'] : $get_form_fields_meta['form_fields']; //phpcs:ignore
					...
					$appointment_end_date = $appointment_booking_fields['bookingpress_appointment_end_date'];
				...
				if ( ! empty( $entry_data ) ) {
					...
					foreach($entry_data as $k => $v){
						...
						$bookingpress_appointment_end_date           = $v['bookingpress_appointment_end_date'];
						$bookingpress_is_next_day_booking 		     = $v['bookingpress_is_next_day_booking'];
						...
						$bookingpress_selected_appointment_date	  	 = $v['bookingpress_selected_appointment_date'];
						$bookingpress_selected_appointment_end_date  = $v['bookingpress_selected_appointment_end_date'];
						$bookingpress_selected_appointment_time	  	 = $v['bookingpress_selected_appointment_time'];
						$bookingpress_selected_appointment_end_time  = $v['bookingpress_selected_appointment_end_time'];
						$bookingpress_is_customer_next_day_booking   = $v['bookingpress_customer_is_next_day_booking'];
					...
					if ( ! empty( $bookingpress_inserted_appointment_ids ) ) {
						...
						$bookingpress_send_email_staff_ids = [];

						$is_staff_active = $bookingpress_pro_staff_members->bookingpress_check_staffmember_module_activation();

						$email_sent_count = 0;
						$is_send_to_customer = true;
						foreach($bookingpress_inserted_appointment_ids as $k2 => $v2){
							do_action( 'bookingpress_after_book_appointment', $v2, $entry_id, $payment_gateway_data );

							if($is_staff_active == 1 && isset($payment_log_data['bookingpress_is_cart']) && $payment_log_data['bookingpress_is_cart'] == '1'){
									
								$appointments_data = $wpdb->get_row( $wpdb->prepare( "SELECT bookingpress_staff_member_id FROM {$tbl_bookingpress_appointment_bookings} WHERE bookingpress_appointment_booking_id = %d", $v2 ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_appointment_bookings is a table name. false alarm

								$staff_member_id = "";

								if ( ! empty( $appointments_data['bookingpress_staff_member_id'] ) ) {
									$staff_member_id = $appointments_data['bookingpress_staff_member_id'];
								}

								if(!empty($staff_member_id) && !in_array($staff_member_id, $bookingpress_send_email_staff_ids)){

									if($email_sent_count > 0){
										$is_send_to_customer = false;
									}
									
									$bookingpress_email_notifications->bookingpress_send_after_payment_log_entry_email_notification( $bookingpress_email_notification_type, $v2, $bookingpress_customer_email, $is_send_to_customer );

									$bookingpress_send_email_staff_ids[] = $appointments_data['bookingpress_staff_member_id'];
								}
								$email_sent_count++;
							}
							else{
		...
		function bookingpress_recalculate_appointment_data_func( $bookingpress_appointment_data = array() ) {
			...
			if ( ! empty( $bookingpress_appointment_details ) && empty($bookingpress_appointment_details['cart_items']) ) {
				...
				if(!empty($payment_gateway) && $payment_gateway != "on-site" && $bookingpress_deposit_payment->bookingpress_check_deposit_payment_module_activation() && !empty($bookingpress_appointment_details['bookingpress_deposit_payment_method']) && ($bookingpress_appointment_details['bookingpress_deposit_payment_method'] == "deposit_or_full_price") && empty($bookingpress_appointment_details['cart_items']) ){
					...
					if(!isset($bookingpress_appointment_details['deposit_payment_type_org'])){
						$bookingpress_appointment_details['deposit_payment_type_org'] = $bookingpress_appointment_details['deposit_payment_type'];
					}

					$deposit_payment_type_before = $bookingpress_appointment_details['deposit_payment_type_org'];
					...
					$total_payable_amount_temp = $total_payable_amount;					
					$total_payable_amount = $bookingpress_deposit_amt;
					if(empty( $coupon_code ) && empty($deposit_payment_type_before)){
						$total_payable_amount = $total_payable_amount_temp;		
					}
				}						
				...
				if ( $bookingpress_coupons->bookingpress_check_coupon_module_activation() && ! empty( $coupon_code ) ) {
				...
				else if( $bookingpress_coupons->bookingpress_check_coupon_module_activation() && empty( $coupon_code )){
					if( !empty($bookingpress_appointment_details['tax_price_display_options']) && $bookingpress_appointment_details['tax_price_display_options'] == "exclude_taxes" ){
						$tax_amount = isset($bookingpress_appointment_details['tax_amount_without_currency']) ? $bookingpress_appointment_details['tax_amount_without_currency'] : 0;
						$total_payable_amount = $total_payable_amount +  $tax_amount;	
					}
				}
		...
		function bookingpress_after_refund_success($response,$bookingpress_refund_data,$appointment_data,$refund_intiate_from ) {
			...
			if(!empty($bookingpress_refund_data) && !empty($appointment_data)) {				
		        ...
		        if($bookingpres_refund_type != 'full') {                    
					...
					$bookingpress_refund_amount = $bookingpress_refund_data['bookingpress_paid_amount'] ? $bookingpress_refund_data['bookingpress_paid_amount'] : 0;		$bookingpress_refund_amount = apply_filters('bookingpress_modify_refund_data_amount', $bookingpress_refund_amount, $bookingpress_payment_log_id);
				}

				if($bookingpress_deposit_amount > 0) {
					$bookingpress_payment_status = 5;
				}
				else if($bookingpress_refund_amount != $bookingpress_refund_data['bookingpress_paid_amount'] && $bookingpres_refund_type != 'full'){
					$bookingpress_payment_status = 5;
				}
				else {
					$bookingpress_payment_status = 3;
				}
