	...
	class bookingpress_pro_appointment Extends BookingPress_Core {
		...
		function bookingpress_get_refund_amount_func() {
			...
			if(!empty($bookingpress_appointment_id) && !empty($bookingpress_payment_id)) {
				...
				$response['refund_amount'] = $BookingPress->bookingpress_price_formatter_with_currency_symbol( $refund_data['refund_amount'], '', false);	
				$response['default_refund_amount'] = $BookingPress->bookingpress_price_formatter_with_currency_symbol( $refund_data['default_refund_amount'], '', false);					
		...
		function bookingpress_modify_edit_appointment_data_func($edit_appointment_data){
			global $wpdb, $BookingPress, $bookingpress_pro_services, $tbl_bookingpress_appointment_bookings, $tbl_bookingpress_payment_logs, $bookingpress_bring_anyone_with_you, $bookingpress_global_options;
		...
		function bookingpress_modify_appointment_booking_fields_func($appointment_booking_field, $entry_data, $bookingpress_appointment_data){
			...
			if(!empty($appointment_booking_field) && !empty($entry_data) ){
				...
				if( current_time('timestamp') > strtotime($bookingpress_appointment_new_date . ' ' . $bookingpress_appointment_new_start_time) && $bookingpress_appointment_allow_customize_timing == 0) {
					$response['variant'] 	= 'error';
					$response['title']   	= esc_html__( 'Error', 'bookingpress-appointment-booking' );
